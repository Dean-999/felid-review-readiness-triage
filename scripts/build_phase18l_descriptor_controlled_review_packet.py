#!/usr/bin/env python3
"""Build descriptor-controlled high-similarity PF-ERI review packets."""
...
DEFAULT_OUTPUT_ROOT = Path("outputs/phase18/phase18l_descriptor_controlled_review_packet")
...
CODEBOOK_ROWS = [
    ...
    {"field": "reviewability_decision", "allowed_value": "uncertain", "definition": "Use only when the reviewer cannot determine reviewability after considering the reason options."},
...
def greedy_similarity_matches(
    ...
    available_low = sorted(low_rows, key=lambda row: to_float(row["descriptor_similarity_percentile"]))
    matches = []
    for high_row in high:
        if not available_low or len(matches) >= pair_count:
            break
        high_similarity = to_float(high_row["descriptor_similarity_percentile"])
        best_idx = min(
            range(len(available_low)),
            key=lambda idx: abs(to_float(available_low[idx]["descriptor_similarity_percentile"]) - high_similarity),
        )
        low_row = available_low.pop(best_idx)
        delta = abs(to_float(low_row["descriptor_similarity_percentile"]) - high_similarity)
...
def row_to_output(
    ...
    return {
        "review_pair_id": f"phase18l_{descriptor_name}_blind_{idx:04d}",
        "descriptor_name": descriptor_name,
        "sample_scope": "descriptor_controlled_high_similarity_admissibility_contrast",
        ...
        "reviewer_id": "",
        "review_timestamp": "",
        "claim_boundary": "Descriptor-controlled reviewability packet; labels are not identity labels.",
    }


def build_descriptor_packet(
    descriptor_name: str,
    features_csv: Path,
    ...
    sim_cutoff = quantile(rows, "descriptor_similarity_percentile", 0.75)
    adm_low = quantile(rows, "pf_eri_admissibility_score", 0.333333)
    adm_high = quantile(rows, "pf_eri_admissibility_score", 0.666667)
    high_similarity = [row for row in rows if to_float(row["descriptor_similarity_percentile"]) >= sim_cutoff]
    high_evidence = [row for row in high_similarity if to_float(row["pf_eri_admissibility_score"]) >= adm_high]
    low_evidence = [row for row in high_similarity if to_float(row["pf_eri_admissibility_score"]) <= adm_low]
    rng = random.Random(random_seed + sum(ord(ch) for ch in descriptor_name))
    rng.shuffle(high_evidence)
    rng.shuffle(low_evidence)
    requested_match_pairs = sample_size // 2
    matches = greedy_similarity_matches(high_evidence, low_evidence, requested_match_pairs)
    if len(matches) < requested_match_pairs:
        raise ValueError(
            f"insufficient descriptor-controlled matches for {descriptor_name}: "
            f"requested {requested_match_pairs}, found {len(matches)}"
        )
    output_rows = []
    idx = 1
    for match_idx, (high_row, low_row, delta) in enumerate(matches, start=1):
        group = f"phase18l_{descriptor_name}_match_{match_idx:04d}"
        output_rows.append(row_to_output(descriptor_name, high_row, by_id, idx, group, "high_admissibility", delta))
        idx += 1
        output_rows.append(row_to_output(descriptor_name, low_row, by_id, idx, group, "low_admissibility", delta))
        idx += 1
    rng.shuffle(output_rows)
    for new_idx, row in enumerate(output_rows, start=1):
        row["review_pair_id"] = f"phase18l_{descriptor_name}_blind_{new_idx:04d}"

    output_dir.mkdir(parents=True, exist_ok=True)
    write_csv(output_dir / "phase18l_descriptor_controlled_review_packet.csv", output_rows, FULL_COLUMNS)
    write_csv(output_dir / "phase18l_blind_review_form.csv", output_rows, BLIND_COLUMNS)
    write_csv(output_dir / "phase18l_reviewability_codebook.csv", CODEBOOK_ROWS, CODEBOOK_COLUMNS)
    audit = {
        ...
        "mean_similarity_match_delta": float(np.mean([delta for _, _, delta in matches])) if matches else 0.0,
        "status": "PASS",
        "claim_boundary": "High-similarity descriptor-controlled PF-ERI admissibility contrast; reviewability labels are not identity labels.",
    }
    write_json(output_dir / "phase18l_descriptor_controlled_review_packet_audit.json", audit)
    return audit


def build_phase18l_descriptor_controlled_review_packet(
    ...
    for descriptor_name in DESCRIPTORS:
        ...
        audits[descriptor_name] = build_descriptor_packet(
            descriptor_name=descriptor_name,
    ...
    write_json(output_root / "phase18l_descriptor_controlled_review_packet_combined_audit.json", combined)
    return combined
...
def main() -> int:
    args = parse_args()
    audit = build_phase18l_descriptor_controlled_review_packet(
        output_root=args.output_root,
        phase18a_manifest=args.phase18a_manifest,
        sample_size=args.sample_size,
        random_seed=args.random_seed,
    )
    print("PASS phase18l descriptor-controlled review packet")
