#!/usr/bin/env python3
"""Build blinded contact sheets for CzechLynx pilot triage review."""
...
OUTPUT_DIR = PROJECT_ROOT / "outputs" / "czechlynx" / "pilot_contact_sheets"
...
def build_contact_sheet(
    ...
    draw.text(
        (PADDING, 8),
        f"CzechLynx pilot contact sheet {sheet_number:02d}",
        fill="black",
        font=title_font,
    )
    ...
    for index, row in enumerate(rows.itertuples(index=False)):
        ...
        label = str(row.pilot_image_id)
        label_y = y0 + THUMB_SIZE + 4
        draw.text((x0, label_y), label, fill="black", font=label_font)
...
def main() -> None:
    ...
    df = df.sort_values("pilot_image_id").reset_index(drop=True)

    if len(df) != IMAGES_PER_SHEET * 8:
        raise ValueError(
            f"Expected 200 pilot images, found {len(df)} in {BLINDED_CSV}"
        )
    ...
    for sheet_idx in range(8):
        ...
        output_path = OUTPUT_DIR / f"czechlynx_pilot_contact_sheet_{sheet_idx + 1:02d}.jpg"
        sheet.save(output_path, quality=90)
        written.append(output_path)

    print(f"Read {len(df)} pilot rows from: {BLINDED_CSV}")
