#!/usr/bin/env python3
"""Audit Phase 11C-1 C3-base positive-weighting package."""
...
OUT = PROJECT_ROOT / "outputs/czechlynx/phase11c/phase11c_c3base_positive_weighting_package_audit.csv"

REQUIRED_CONFIGS = {
    "config_phase11c_c3base_sqrt.yaml": ("P11C_C3base_sqrt", 0.0, 0.5),
    "config_phase11c_c3base_a05_g065.yaml": ("P11C_C3base_a05_g065", 0.05, 0.65),
    "config_phase11c_c3base_a10_g065.yaml": ("P11C_C3base_a10_g065", 0.10, 0.65),
}
REQUIRED_FILES = [
    "train_phase11_pair_weighted_metric_learning.py",
    "run_phase11c_c3base_positive_weighting.py",
    *REQUIRED_CONFIGS,
]
FORBIDDEN_TERMS = [
    "second-review",
...
def main() -> None:
    ...
    add(rows, "training_script_supports_alpha_gamma", "alpha_gamma" in train_text, "alpha_gamma")
    ...
    if PAIR_TABLE.exists():
        ...
        add(rows, "c3_base_group_in_pair_table", "C3_quality_proxy_matched_identity" in groups, "C3_quality_proxy_matched_identity")
    ...
    for config_name, (expected_group, expected_alpha, expected_gamma) in REQUIRED_CONFIGS.items():
        ...
        add(
            rows,
            f"{prefix}_base_group_c3",
            str(config.get("base_group_name")) == "C3_quality_proxy_matched_identity",
            str(config.get("base_group_name")),
        )
        add(
            rows,
            f"{prefix}_does_not_use_h3_base",
        ...
        add(rows, f"{prefix}_alpha_gamma_transform", str(loss.get("positive_weight_transform")) == "alpha_gamma", str(loss.get("positive_weight_transform")))
    ...
    add(rows, "all_required_alpha_gamma_present", len(observed_params) == 3, f"observed={observed_params}")

    scanned_files = [PACKAGE_DIR / name for name in REQUIRED_FILES if (PACKAGE_DIR / name).exists()]
    scanned_files += [PROJECT_ROOT / "docs/phase11/phase11c_c3base_positive_weighting.md"]
    ...
    print(f"Phase 11C-1 package audit: PASS={pass_count} FAIL={fail_count} output={OUT.relative_to(PROJECT_ROOT)}")
    if fail_count:
        raise SystemExit(1)
