#!/usr/bin/env python3
"""Streamlit review app for Phase17L multi-source strict Bobcat clarity queue."""
...
DEFAULT_SOURCE_CSV = (
    PROJECT_ROOT
    / "outputs/phase17/phase17l_multisource_strict_clarity_queue/"
    / "phase17l_bobcat_strict_clarity_review_queue_5000.csv"
)
SOURCE_CSV = Path(os.environ.get("PHASE17L_CLARITY_SOURCE_CSV", DEFAULT_SOURCE_CSV))
OUTPUT_DIR = Path(
    os.environ.get(
        "PHASE17L_CLARITY_OUTPUT_DIR",
        PROJECT_ROOT / "outputs/phase17/phase17l_multisource_strict_clarity_review",
    )
)
WORKING_CSV = OUTPUT_DIR / "phase17l_bobcat_strict_clarity_review_working.csv"
...
def completed_mask(frame: pd.DataFrame) -> pd.Series:
    return frame["phase17l_clarity_gate_decision"].isin(["clear", "not_clear"])
...
def save_decision(frame: pd.DataFrame, candidate_id: str, decision: str, reject_reason: str, notes: str) -> str:
    ...
    frame.at[row_index, "phase17l_clarity_gate_decision"] = decision
    frame.at[row_index, "phase17l_clarity_reject_reason"] = reject_reason if decision == "not_clear" else ""
    frame.at[row_index, "phase17l_clarity_notes"] = notes
    frame.at[row_index, "phase17l_clarity_audited_at_utc"] = datetime.now(timezone.utc).isoformat(timespec="seconds")
    save_data(frame)
...
def filtered_frame(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    source = st.sidebar.selectbox("source", ["all", *sorted(v for v in out["phase17l_source"].unique() if v)])
    tier = st.sidebar.selectbox("proxy tier", ["all", *sorted(v for v in out["phase17l_strict_proxy_tier"].unique() if v)])
    if status == "pending":
        out = out[~completed_mask(out)]
    elif status == "completed":
        out = out[completed_mask(out)]
    elif status in {"clear", "not_clear"}:
        out = out[out["phase17l_clarity_gate_decision"].eq(status)]
    if source != "all":
        out = out[out["phase17l_source"].eq(source)]
    if tier != "all":
        out = out[out["phase17l_strict_proxy_tier"].eq(tier)]
    return out


def main() -> None:
    st.set_page_config(page_title="Phase17L Bobcat Strict Clarity", layout="wide")
    st.title("Phase17L Bobcat Strict Clarity Review")
    st.caption("CLEAR = 100% visually clear and comparable. If unsure, NOT CLEAR.")

    ensure_working_csv()
    data = load_data(str(WORKING_CSV))
    complete = completed_mask(data)
    clear_count = int(data["phase17l_clarity_gate_decision"].eq("clear").sum())
    not_clear_count = int(data["phase17l_clarity_gate_decision"].eq("not_clear").sum())
    ...
    if "phase17l_current_id" not in st.session_state:
        st.session_state.phase17l_current_id = query_candidate if query_candidate in candidate_ids else candidate_ids[0]
    if st.session_state.phase17l_current_id not in candidate_ids:
        st.session_state.phase17l_current_id = query_candidate if query_candidate in candidate_ids else candidate_ids[0]

    selected_id = st.sidebar.selectbox(
        "candidate_id",
        candidate_ids,
        index=candidate_ids.index(st.session_state.phase17l_current_id),
        key="phase17l_candidate_selectbox",
    )
    st.session_state.phase17l_current_id = selected_id
    ...
    existing_reason = str(row.get("phase17l_clarity_reject_reason", ""))
    existing_notes = str(row.get("phase17l_clarity_notes", ""))
    ...
    with left:
        ...
        if row.get("source_uri", ""):
            st.link_button("Open source record", row.get("source_uri", ""))

    with right:
        current = row.get("phase17l_clarity_gate_decision", "")
        if current == "clear":
        ...
        st.markdown("- CLEAR only if the bobcat is sharp, large enough, high contrast, and comparable.")
        st.markdown("- Reject tiny/far, blur, compression, low contrast, occlusion, sign/dead-only.")
        st.markdown("- If unsure, NOT CLEAR.")

        st.markdown("### Proxy")
        st.write(f"source: `{row.get('phase17l_source', '')}`")
        st.write(f"score: `{row.get('phase17l_strict_clarity_proxy_score', '')}`")
        st.write(f"tier: `{row.get('phase17l_strict_proxy_tier', '')}`")
        st.write(f"reject proxy: `{row.get('phase17l_proxy_reject_reasons', '') or 'none'}`")
        ...
        with clear_col:
            if st.button("CLEAR", type="primary", use_container_width=True):
                ...
                if next_id:
                    st.session_state.phase17l_current_id = next_id
