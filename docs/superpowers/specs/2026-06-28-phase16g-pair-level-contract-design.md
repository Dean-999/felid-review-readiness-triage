# Phase16G Pair-Level Evidence Contract Design

Date: 2026-06-28

## Decision

Phase16G is a pair-level evidence-table contract and CzechLynx known-ID prototype, not the final calibrated review-router training phase.
...
Phase16G may build deterministic tables, audits, controls, and a CzechLynx-only validation-ready prototype. It must not claim a final model, Bobcat identity accuracy, or automatic identity assignment.
...
Phase16E and Phase16F prepare image-level and candidate-level evidence foundations. They do not freeze the final scientific dataset alone. Project rules say individual Re-ID is a pairwise evidence problem, so the next defensible modeling unit is a pair-level table that connects image evidence to candidate comparison risk.

Phase15 showed that calibrated nonlinear candidate/ranker features contain signal beyond descriptor-only baselines, but also that hand-written gates and simple top-k improvements are not enough for the final claim. Phase16G therefore prepares the data contract, controls, and split discipline needed before Phase16H model training.
...
- Define acceptance gates for moving from Phase16G to Phase16H.
...
### Phase16G-A: Pair Feature Contract
...
### Phase16G-B: CzechLynx Known-ID Prototype

Build a CzechLynx pair table from Phase16F-selected candidates plus designated low-evidence stress rows when available.
...
```text
outputs/phase16/phase16g_pair_contract/phase16g_pair_feature_schema.json
outputs/phase16/phase16g_czechlynx_pair_prototype/phase16g_czechlynx_pair_table.csv
outputs/phase16/phase16g_czechlynx_pair_prototype/phase16g_czechlynx_pair_audit.json
outputs/phase16/phase16g_czechlynx_pair_prototype/phase16g_czechlynx_control_summary.csv
outputs/phase16/phase16g_czechlynx_pair_prototype/phase16g_czechlynx_split_summary.csv
```
...
### Phase16G-C: Bobcat Transfer-Stress Receiver

After Bobcat Phase16E and Phase16F are complete, build a Bobcat pair table using the same schema.
...
## Acceptance Gates Before Phase16H

Phase16H calibrated review-router modeling may start only after these gates pass:

1. Phase16G schema is stable and versioned.
...
7. Documentation states that Phase16G is not identity assignment and not final review-router training.

## Algorithm Basis For Phase16H

When Phase16H begins, the preferred model family should be calibrated, interpretable, and control-friendly:
...
| Risk | Mitigation |
| --- | --- |
| Premature model training | Keep Phase16G to schema, table builder, audits, and controls. |
...
- The design follows the active Phase16 plan by keeping Phase16G after Phase16F and before Phase16H.
...
- It includes required controls from the Phase16 strategy: descriptor-only, quality-only, random same-size, laterality/source-tier/duplicate diagnostics, and manual audit hooks.
