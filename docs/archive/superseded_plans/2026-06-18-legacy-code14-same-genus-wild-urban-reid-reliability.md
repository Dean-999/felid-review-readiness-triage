# legacy-code14 Same-Genus Wild-Urban Re-ID Reliability Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Build the legacy-code14 implementation layer that tests PF-ERI as a same-genus wild-to-urban Lynx evidence reliability model, organized as image-level evidence shift, pair-level comparability shift, and retrieval/review contamination.
...
- Create: `scripts/audit_legacy-code14_uwin_bobcat_inventory.py`
...
- Create: `scripts/build_legacy-code14_cross_context_evidence_table.py`
...
- Create: `scripts/analyze_legacy-code14_cross_context_shift.py`
  - Computes standardized mean differences, distribution distances, domain-classifier separability, and bootstrap intervals.
- Create: `scripts/build_legacy-code14_pair_comparability_shift_table.py`
...
- Create: `scripts/analyze_legacy-code14_pair_comparability_shift.py`
...
- Create: `docs/legacy-code14/legacy-code14_implementation_status.md`
  - Tracks what has been implemented, what data are missing, and which claims are currently allowed.
- Modify: `docs/README.md`
  - Keep legacy-code14 as active roadmap and point to the implementation status file after scripts exist.
- Modify: `README.md`
  - Update current pending status after each legacy-code14 module completes.
...
def main() -> int:
    ...
    parser.add_argument("--output", default="outputs/legacy-code14/uwin_bobcat_inventory.csv")
    ...
    print(f"PASS legacy-code14 UWIN inventory rows={len(rows)} output={out}")
    return 0
...
```bash
python scripts/audit_legacy-code14_uwin_bobcat_inventory.py --root data
```

Expected:

```text
PASS legacy-code14 UWIN inventory rows=<n> output=outputs/legacy-code14/uwin_bobcat_inventory.csv
```

- [ ] **Step 4: Record claim boundary**

Update `docs/legacy-code14/legacy-code14_implementation_status.md` with:

```markdown
# legacy-code14 Implementation Status

Date: 2026-06-18

## Current Claim Boundary

legacy-code14 can only claim UWIN bobcat field-readiness or review-readiness until individual labels or human-audited pair labels are verified.
...
**Files:**
- Create: `scripts/build_legacy-code14_cross_context_evidence_table.py`
- Modify: `docs/legacy-code14/legacy-code14_implementation_status.md`
...
```bash
python scripts/build_legacy-code14_cross_context_evidence_table.py
```

Expected:

```text
PASS legacy-code14 cross-context evidence table rows=<n> contexts=<CzechLynx,UWIN_bobcat>
```
...
```bash
python scripts/analyze_legacy-code14_cross_context_shift.py
```

Expected:

```text
PASS legacy-code14 cross-context shift features=<n> domain_auc=<value>
```

- [ ] **Step 4: Report allowed interpretation**

Write in `docs/legacy-code14/legacy-code14_implementation_status.md`:
...
```text
PASS legacy-code14 UWIN pair audit manifest pairs=<n> strata=<k>
```
...
Update `README.md` with the latest legacy-code14 status after tasks finish.
...
- [ ] legacy-code14 is the first active implementation direction.
