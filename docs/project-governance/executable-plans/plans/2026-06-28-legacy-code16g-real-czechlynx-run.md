# legacy-code16g Real CzechLynx Pair Table Run Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Run legacy-code16g on real CzechLynx data by bridging legacy-code16f selected images to legacy-code15 descriptor candidate pairs, then generating audited pair-level tables without training a model.

**Architecture:** Add a focused adapter that normalizes CzechLynx paths and converts the real legacy-code16f manifest plus legacy-code15 routing/top-k pairs into the generic inputs expected by the existing legacy-code16g prototype builder. Keep the existing schema builder, audit script, and prototype builder as the core execution layer; the new code only adapts real repository outputs and writes a run report.

**Tech Stack:** Python standard library, pandas, unittest, existing legacy-code16g scripts.
...
```text
outputs/legacy-code16/legacy-code16f_czechlynx_constrained_selection/legacy-code16f_czechlynx_selected_3000_manifest.csv
outputs/legacy-code15/hybrid_routing_policy/legacy-code15_czechlynx_candidate_routing_input.csv
outputs/legacy-code16/leakage_pressure/legacy-code16_leakage_pressure_pair_audit.csv
outputs/legacy-code16/laterality_aware_pair_audit/legacy-code16_laterality_aware_pair_table.csv
```

Why an adapter is needed:

```text
legacy-code16f selected image rows:
candidate_id, image_key, legacy-code16f_selection_bucket, iqa_quality_proxy_score,
clip_side_view_score, clip_viewpoint_label, legacy-code16f_balance_group

legacy-code15 descriptor pair rows:
...
```text
legacy-code16f image_key:
CzechLynx/foe_bohemia/lynx_120/20356_lynx_120.jpg

legacy-code15 query_image_path:
.../data/raw/czechlynx/CzechLynx/foe_bohemia/lynx_120/20356_lynx_120.jpg
```

## File Structure

- Create `scripts/run_legacy-code16g_real_czechlynx_pair_table.py`: real-data adapter and orchestrator.
- Create `tests/test_legacy-code16g_real_czechlynx_pair_table.py`: tests path normalization, selected-image conversion, descriptor-pair conversion, optional flag joins, and end-to-end fixture run.
- Modify `docs/legacy-code16/README.md`: add the real CzechLynx legacy-code16g run command and output paths.
...
```text
outputs/legacy-code16/legacy-code16g_czechlynx_real_pair_table/legacy-code16g_czechlynx_pair_table.csv
outputs/legacy-code16/legacy-code16g_czechlynx_real_pair_table/legacy-code16g_czechlynx_pair_audit.json
outputs/legacy-code16/legacy-code16g_czechlynx_real_pair_table/legacy-code16g_czechlynx_control_summary.csv
outputs/legacy-code16/legacy-code16g_czechlynx_real_pair_table/legacy-code16g_czechlynx_split_summary.csv
outputs/legacy-code16/legacy-code16g_czechlynx_real_pair_table/legacy-code16g_czechlynx_run_summary.json
outputs/legacy-code16/legacy-code16g_czechlynx_real_pair_table/legacy-code16g_czechlynx_run_report.md
```
...
```text
legacy-code16g produced an audited CzechLynx known-ID pair-level evidence table for later legacy-code16h calibrated review-router modeling.
```
...
**Files:**
- Create: `tests/test_legacy-code16g_real_czechlynx_pair_table.py`
...
from scripts.run_legacy-code16g_real_czechlynx_pair_table import (
...
class legacy-code16grealczechlynxpairtabletest(unittest.TestCase):
    ...
    def test_build_selected_image_table_maps_real_legacy-code16f_columns(self):
        selected = pd.DataFrame(
            [
                {
                    "candidate_id": "p16e_1",
                    "image_key": "CzechLynx/site/lynx_001/a.jpg",
                    "legacy-code16f_selection_bucket": "strict_core",
                    "legacy-code16f_balance_group": "lynx_001",
                    "iqa_quality_proxy_score": 0.9,
                    "clip_side_view_score": 0.8,
                    "clip_viewpoint_label": "left_side",
                    "source_name": "czechlynx_real_all",
                    "legacy-code16f_selection_rank": 1,
                }
            ]
        )
        ...
        self.assertEqual(out.loc[0, "legacy-code16f_tier"], "strict_core")
            ...
            [
                {
                    "query_image_path": "/root/CzechLynx/site/lynx_001/a.jpg",
                    "candidate_image_path": "/root/CzechLynx/site/lynx_001/b.jpg",
                    "legacy-code16_site_leakage_pressure": True,
                }
            ]
        )
            ...
            [
                {
                    "query_image_path": "/root/CzechLynx/site/lynx_001/a.jpg",
                    "candidate_image_path": "/root/CzechLynx/site/lynx_001/b.jpg",
                    "legacy-code16_pair_side_relation": "same_side",
    ...
    def test_run_real_czechlynx_pair_table_writes_expected_outputs(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertTrue((output_dir / "legacy-code16g_czechlynx_pair_table.csv").exists())
            self.assertTrue((output_dir / "legacy-code16g_czechlynx_run_report.md").exists())
...
PYTHONDONTWRITEBYTECODE=1 $PY -m unittest tests/test_legacy-code16g_real_czechlynx_pair_table.py -v
```

Expected:

```text
ModuleNotFoundError: No module named 'scripts.run_legacy-code16g_real_czechlynx_pair_table'
```

## Task 2: Implement The Real CzechLynx Runner

**Files:**
- Create: `scripts/run_legacy-code16g_real_czechlynx_pair_table.py`
- Test: `tests/test_legacy-code16g_real_czechlynx_pair_table.py`
...
```python
#!/usr/bin/env python3
"""Run legacy-code16g CzechLynx pair-table construction on real repository outputs."""
...
from scripts.build_legacy-code16g_czechlynx_pair_prototype import build_pair_rows, write_outputs
from scripts.build_legacy-code16g_pair_feature_schema import write_schema


DEFAULT_SELECTED = Path("outputs/legacy-code16/legacy-code16f_czechlynx_constrained_selection/legacy-code16f_czechlynx_selected_3000_manifest.csv")
DEFAULT_ROUTING = Path("outputs/legacy-code15/hybrid_routing_policy/legacy-code15_czechlynx_candidate_routing_input.csv")
DEFAULT_LEAKAGE = Path("outputs/legacy-code16/leakage_pressure/legacy-code16_leakage_pressure_pair_audit.csv")
DEFAULT_LATERALITY = Path("outputs/legacy-code16/laterality_aware_pair_audit/legacy-code16_laterality_aware_pair_table.csv")
DEFAULT_OUTPUT_DIR = Path("outputs/legacy-code16/legacy-code16g_czechlynx_real_pair_table")
...
def build_selected_image_table(selected: pd.DataFrame) -> pd.DataFrame:
    ...
    out["identity_id"] = _string(selected, "legacy-code16f_balance_group")
    out["legacy-code16f_tier"] = _string(selected, "legacy-code16f_selection_bucket")
    ...
    out["legacy-code16f_selection_rank"] = _numeric(selected, "legacy-code16f_selection_rank")

    if out["image_id"].duplicated().any():
        duplicate_count = int(out["image_id"].duplicated().sum())
        raise ValueError(f"legacy-code16f selected image_id must be unique; duplicate_count={duplicate_count}")
    if out["identity_id"].eq("").any():
        missing_count = int(out["identity_id"].eq("").sum())
        raise ValueError(f"legacy-code16f selected rows missing legacy-code16f_balance_group; missing_count={missing_count}")
    return out
...
def _optional_pair_flags(frame: pd.DataFrame | None) -> pd.DataFrame:
    ...
    for column in [
        "legacy-code16_site_leakage_pressure",
        "legacy-code16_pair_side_relation",
...
def build_descriptor_pairs_for_selected(
    ...
    if not leakage_flags.empty and "legacy-code16_site_leakage_pressure" in leakage_flags.columns:
        out = out.merge(
            leakage_flags[["pair_join_key", "legacy-code16_site_leakage_pressure"]],
            on="pair_join_key",
            how="left",
        )
        out["source_leakage_pressure_flag"] = out["legacy-code16_site_leakage_pressure"].fillna(False).astype(bool)
        out = out.drop(columns=["legacy-code16_site_leakage_pressure"])

    laterality_flags = _optional_pair_flags(laterality)
    if not laterality_flags.empty:
        keep_columns = [
            column
            for column in ["pair_join_key", "legacy-code16_pair_side_relation", "side_direction_compatibility"]
            if column in laterality_flags.columns
        ]
        out = out.merge(laterality_flags[keep_columns], on="pair_join_key", how="left")
        if "legacy-code16_pair_side_relation" in out.columns:
            out["laterality_relation"] = out["legacy-code16_pair_side_relation"].fillna("unknown").astype(str)
            out = out.drop(columns=["legacy-code16_pair_side_relation"])
...
def write_run_report(summary: dict[str, object], output_dir: Path) -> None:
    lines = [
        "# legacy-code16g CzechLynx Real Pair Table Run",
        "",
        "This run bridges legacy-code16f CzechLynx selected images to legacy-code15 descriptor candidate pairs.",
        "It creates an audited pair-level evidence table for later legacy-code16h modeling.",
        "",
        "## Summary",
        "",
    ...
    (output_dir / "legacy-code16g_czechlynx_run_report.md").write_text("\n".join(lines))
...
def run_real_czechlynx_pair_table(
    ...
    pair_table = build_pair_rows(
        selected_images,
        descriptor_pairs,
        control_regime="legacy-code16f_selected_real_czechlynx",
    )

    output_dir.mkdir(parents=True, exist_ok=True)
    write_schema(output_dir / "legacy-code16g_pair_feature_schema.json")
    ...
    (output_dir / "legacy-code16g_czechlynx_run_summary.json").write_text(
...
## Task 3: Run Real CzechLynx legacy-code16g

**Files:**
- Uses: `scripts/run_legacy-code16g_real_czechlynx_pair_table.py`
- Writes ignored outputs under: `outputs/legacy-code16/legacy-code16g_czechlynx_real_pair_table/`
...
```bash
PY=/Users/dshen/.cache/codex-runtimes/codex-primary-runtime/dependencies/python/bin/python3
$PY scripts/run_legacy-code16g_real_czechlynx_pair_table.py
```
...
Wrote outputs/legacy-code16/legacy-code16g_czechlynx_real_pair_table
```
...
```bash
cat outputs/legacy-code16/legacy-code16g_czechlynx_real_pair_table/legacy-code16g_czechlynx_run_summary.json
```
...
$PY - <<'PY'
import pandas as pd
path = "outputs/legacy-code16/legacy-code16g_czechlynx_real_pair_table/legacy-code16g_czechlynx_pair_table.csv"
df = pd.read_csv(path)
print("rows", len(df))
...
**Files:**
- Modify: `docs/legacy-code16/README.md`
- Modify: `scripts/README.md`

- [ ] **Step 1: Update `docs/legacy-code16/README.md`**

Add this section after the legacy-code16g planning boundary:
...
Add this bullet near other legacy-code16g scripts:

```markdown
- `run_legacy-code16g_real_czechlynx_pair_table.py` - bridges the real legacy-code16f
  CzechLynx selected 3000 manifest to legacy-code15 descriptor candidate pairs,
  joins optional leakage/laterality flags, and writes the audited legacy-code16g
...
python3 - <<'PY'
from pathlib import Path
for path in [Path("docs/legacy-code16/README.md"), Path("scripts/README.md")]:
    text = path.read_text()
    assert "run_legacy-code16g_real_czechlynx_pair_table.py" in text
    assert "legacy-code16h" in text or "legacy-code16h" in text
print("legacy-code16g real-run docs PASS")
PY
```
...
```text
legacy-code16g real-run docs PASS
```
...
- [ ] **Step 1: Run focused legacy-code16g tests**
...
PYTHONDONTWRITEBYTECODE=1 $PY -m unittest \
  tests/test_legacy-code16g_pair_feature_schema.py \
  tests/test_legacy-code16g_pair_table_audit.py \
  tests/test_legacy-code16g_czechlynx_pair_prototype.py \
  tests/test_legacy-code16g_real_czechlynx_pair_table.py \
  -v
```
...
- Spec coverage: This plan bridges real legacy-code16f selected rows to legacy-code15 descriptor candidate pairs, preserves leakage/laterality flags when present, writes audited legacy-code16g outputs, and documents the run.
- Placeholder scan: No plan-for-later markers or unspecified test instructions remain.
- Type consistency: The adapter emits `image_id`, `identity_id`, `iqa_score`, `side_probability`, `query_image_id`, `candidate_image_id`, `descriptor_similarity`, `split_group`, `source_leakage_pressure_flag`, and `laterality_relation`, matching the existing legacy-code16g prototype builder.
