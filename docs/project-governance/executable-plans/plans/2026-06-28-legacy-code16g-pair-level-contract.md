# legacy-code16g Pair-Level Contract Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Build legacy-code16g as a deterministic pair-level evidence contract and CzechLynx known-ID prototype without training the final review-router.

**Architecture:** Add a schema-first contract under `docs/legacy-code16/`, then implement focused Python scripts that build and audit pair-level tables from legacy-code16f/legacy-code15 artifacts. The builder must keep image evidence, pair comparability, descriptor support, conflict features, labels, controls, and split fields separate so legacy-code16h can train a calibrated review-router later.
...
- Create `docs/legacy-code16/legacy-code16g_pair_level_contract.md`: human-readable schema, phase boundary, allowed claims, and legacy-code16h gates.
- Create `scripts/build_legacy-code16g_pair_feature_schema.py`: writes `legacy-code16g_pair_feature_schema.json` and validates required column groups.
- Create `scripts/build_legacy-code16g_czechlynx_pair_prototype.py`: builds CzechLynx pair rows from available legacy-code16f/legacy-code15 inputs, controls, labels, and split groups.
- Create `scripts/audit_legacy-code16g_pair_table.py`: reusable audit for required columns, unique pair IDs, label boundaries, split leakage, and Bobcat claim boundaries.
- Create `tests/test_legacy-code16g_pair_feature_schema.py`: unit tests for schema content and grouped columns.
- Create `tests/test_legacy-code16g_czechlynx_pair_prototype.py`: fixture-level tests for CzechLynx prototype behavior.
- Create `tests/test_legacy-code16g_pair_table_audit.py`: tests audit failures and PASS cases.
- Modify `docs/legacy-code16/README.md`: add legacy-code16g as contract/prototype and legacy-code16h as future modeling.
- Modify `scripts/README.md`: add legacy-code16g entry points after legacy-code16f.

## Task 1: Document The legacy-code16g Contract

**Files:**
- Create: `docs/legacy-code16/legacy-code16g_pair_level_contract.md`
- Modify: `docs/legacy-code16/README.md`

- [ ] **Step 1: Create the contract document**

Create `docs/legacy-code16/legacy-code16g_pair_level_contract.md` with this content:
...
legacy-code16g defines the pair-level evidence table used after legacy-code16f constrained selection and before legacy-code16h calibrated review-router modeling.

legacy-code16g is not final model training. It is not identity assignment. It does not report Bobcat identity accuracy without verified labels or audited same/different pair labels.
...
Modify `docs/legacy-code16/README.md` after the current "Next Step" section to add:

````markdown
## legacy-code16g Planning Boundary

legacy-code16g is planned as a pair-level evidence contract and CzechLynx known-ID prototype:
...
legacy-code16g should not train the final review-router. Formal calibrated modeling belongs in legacy-code16h after Bobcat legacy-code16e/16F and manual audit calibration are available.
```
...
for path in [
    Path("docs/legacy-code16/README.md"),
    Path("docs/legacy-code16/legacy-code16g_pair_level_contract.md"),
]:
    text = path.read_text()
    assert "legacy-code16g" in text
    assert "identity assignment" in text or "identity accuracy" in text
print("legacy-code16g docs smoke check PASS")
PY
```
...
```text
legacy-code16g docs smoke check PASS
```

- [ ] **Step 4: Commit**

```bash
git add docs/legacy-code16/README.md docs/legacy-code16/legacy-code16g_pair_level_contract.md
git commit -m "docs: define legacy-code16g pair-level contract"
```

## Task 2: Add The Pair Feature Schema Builder

**Files:**
- Create: `scripts/build_legacy-code16g_pair_feature_schema.py`
- Create: `tests/test_legacy-code16g_pair_feature_schema.py`
...
from scripts.build_legacy-code16g_pair_feature_schema import build_schema, write_schema


class legacy-code16gpairfeatureschematest(unittest.TestCase):
    ...
    def test_write_schema_outputs_valid_json(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(loaded["phase"], "legacy-code16g")
...
```bash
PYTHONDONTWRITEBYTECODE=1 python3 -m unittest tests/test_legacy-code16g_pair_feature_schema.py -v
```

Expected:

```text
ModuleNotFoundError: No module named 'scripts.build_legacy-code16g_pair_feature_schema'
```
...
```python
#!/usr/bin/env python3
"""Build the legacy-code16g pair-level feature schema."""
...
DEFAULT_OUTPUT = Path("outputs/legacy-code16/legacy-code16g_pair_contract/legacy-code16g_pair_feature_schema.json")


def build_schema() -> dict[str, object]:
    return {
        "phase": "legacy-code16g",
        "schema_version": "1.0",
        "claim_boundary": {
        ...
        "column_groups": [
            ...
            {"name": "control_features", "columns": ["control_regime", "duplicate_or_near_duplicate_flag", "source_leakage_pressure_flag", "query_source_tier", "candidate_source_tier", "query_legacy-code16f_tier", "candidate_legacy-code16f_tier"]},
...
```bash
python3 scripts/build_legacy-code16g_pair_feature_schema.py
```

Expected:

```text
Wrote outputs/legacy-code16/legacy-code16g_pair_contract/legacy-code16g_pair_feature_schema.json
```

- [ ] **Step 6: Commit**

```bash
git add scripts/build_legacy-code16g_pair_feature_schema.py tests/test_legacy-code16g_pair_feature_schema.py
git commit -m "feat: add legacy-code16g pair schema builder"
```

## Task 3: Add The Pair Table Audit

**Files:**
- Create: `scripts/audit_legacy-code16g_pair_table.py`
- Create: `tests/test_legacy-code16g_pair_table_audit.py`
...
from scripts.audit_legacy-code16g_pair_table import audit_pair_table
...
class legacy-code16gpairtableaudittest(unittest.TestCase):
...
```bash
PYTHONDONTWRITEBYTECODE=1 python3 -m unittest tests/test_legacy-code16g_pair_table_audit.py -v
```

Expected:

```text
ModuleNotFoundError: No module named 'scripts.audit_legacy-code16g_pair_table'
```
...
```python
#!/usr/bin/env python3
"""Audit legacy-code16g pair-level tables."""
...
```bash
git add scripts/audit_legacy-code16g_pair_table.py tests/test_legacy-code16g_pair_table_audit.py
git commit -m "feat: add legacy-code16g pair table audit"
```

## Task 4: Build The CzechLynx Pair Prototype

**Files:**
- Create: `scripts/build_legacy-code16g_czechlynx_pair_prototype.py`
- Create: `tests/test_legacy-code16g_czechlynx_pair_prototype.py`
...
from scripts.build_legacy-code16g_czechlynx_pair_prototype import build_pair_rows


class legacy-code16gczechlynxpairprototypetest(unittest.TestCase):
    def test_build_pair_rows_creates_known_id_labels_and_controls(self):
        selected = pd.DataFrame(
            [
                {"image_id": "a", "identity_id": "id1", "legacy-code16f_tier": "strict_core", "iqa_score": 0.9, "side_probability": 0.8},
                {"image_id": "b", "identity_id": "id1", "legacy-code16f_tier": "strict_core", "iqa_score": 0.8, "side_probability": 0.7},
                {"image_id": "c", "identity_id": "id2", "legacy-code16f_tier": "balanced_only", "iqa_score": 0.7, "side_probability": 0.6},
            ]
        )
        ...
        rows = build_pair_rows(selected, descriptor, control_regime="legacy-code16f_selected")
        ...
        self.assertEqual(same["control_regime"], "legacy-code16f_selected")
...
```bash
PYTHONDONTWRITEBYTECODE=1 python3 -m unittest tests/test_legacy-code16g_czechlynx_pair_prototype.py -v
```

Expected:

```text
ModuleNotFoundError: No module named 'scripts.build_legacy-code16g_czechlynx_pair_prototype'
```
...
```python
#!/usr/bin/env python3
"""Build a legacy-code16g CzechLynx known-ID pair prototype table."""
...
DEFAULT_OUTPUT_DIR = Path("outputs/legacy-code16/legacy-code16g_czechlynx_pair_prototype")
        ...
        rows.append(
            {
                ...
                "query_legacy-code16f_tier": query.get("legacy-code16f_tier", ""),
                "candidate_legacy-code16f_tier": candidate.get("legacy-code16f_tier", ""),
...
def main() -> int:
    ...
    parser.add_argument("--control-regime", default="legacy-code16f_selected")
    args = parser.parse_args()
    ...
    pair_table.to_csv(args.output_dir / "legacy-code16g_czechlynx_pair_table.csv", index=False)
    pd.DataFrame([audit]).to_json(args.output_dir / "legacy-code16g_czechlynx_pair_audit.json", orient="records", indent=2)
    pair_table.groupby("control_regime").size().rename("pair_count").reset_index().to_csv(args.output_dir / "legacy-code16g_czechlynx_control_summary.csv", index=False)
    pair_table.groupby("split_group").size().rename("pair_count").reset_index().to_csv(args.output_dir / "legacy-code16g_czechlynx_split_summary.csv", index=False)
...
```bash
git add scripts/build_legacy-code16g_czechlynx_pair_prototype.py tests/test_legacy-code16g_czechlynx_pair_prototype.py
git commit -m "feat: add legacy-code16g CzechLynx pair prototype"
```

## Task 5: Register legacy-code16g Entry Points
...
Add this section to `scripts/README.md` near the legacy-code16 entries:

````markdown
### legacy-code16g Pair-Level Contract

```bash
python3 scripts/build_legacy-code16g_pair_feature_schema.py
python3 scripts/audit_legacy-code16g_pair_table.py --input outputs/legacy-code16/legacy-code16g_czechlynx_pair_prototype/legacy-code16g_czechlynx_pair_table.csv --output outputs/legacy-code16/legacy-code16g_czechlynx_pair_prototype/legacy-code16g_czechlynx_pair_audit.json
````

legacy-code16g prepares pair-level tables and audits for later legacy-code16h review-router modeling. It does not train the final model.
```
...
```bash
git add docs/legacy-code16/README.md scripts/README.md
git commit -m "docs: register legacy-code16g pair contract workflow"
```

## Self-Review

- Spec coverage: The plan implements the legacy-code16g-A schema contract, legacy-code16g-B CzechLynx known-ID prototype, reusable audit boundaries, docs, and legacy-code16h gates.
