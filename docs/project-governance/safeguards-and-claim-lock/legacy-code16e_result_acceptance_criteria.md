...
legacy-code16e output is pretrained model scoring and candidate evidence signals. It is not final high-confidence data and not identity prediction.
...
## legacy-code16f Soft Eligibility
...
```bash
python3 scripts/recalibrate_legacy-code16e_candidate_scores.py \
  --input outputs/legacy-code16/legacy-code16e_candidate_model_filter/legacy-code16e_candidate_model_filter_scores.csv \
  --output-dir outputs/legacy-code16/legacy-code16e_recalibrated_candidate_scores
```
...
Tier 2 expands scoring margin. It is not clean Re-ID data. It must pass legacy-code16e scoring, legacy-code16f constrained selection, laterality checks, and manual audit before final high-confidence use.

## Go / No-Go

Go to legacy-code16f if:
...
```text
legacy-code16e extracted auditable pretrained quality, viewpoint, and optional pose structural proxies for candidate evidence scoring.
```
