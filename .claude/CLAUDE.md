...
## Codebase Intelligence for felid-review-readiness-triage (Repowise)

Indexed by [Repowise](https://repowise.dev). Last indexed: 2026-07-17 (commit eb75d77). Confidence: 100%.
...
- `scripts/package_phase16e_candidate_model_filter_colab.py` — 3 commits/90d (99.8th %ile)
- `tests/test_phase18_pipeline.py` — 3 commits/90d (99.3th %ile)

### Code health
Three co-equal signals: defect risk 9.32/10 avg, hotspot health 4.88/10 (stable), worst `sessions/hooks/shared_state.js` at 3.15/10 · maintainability 9.43/10 · performance risk 169 open static I/O-in-loop / N+1 findings. Detail: `get_health()`.

Critical files:
- `scripts/package_phase16e_candidate_model_filter_colab.py` — change entropy — impact −3.0
- `tests/test_phase18_pipeline.py` — god class (Phase18PipelineTests) — impact −1.0
- `scripts/prepare_v2_image_allocation_strata.py` — brain method (prepare_strata) — impact −0.6
- `scripts/streamlit_pair_review_common.py` — brain method (run_pair_review_app) — impact −0.6
