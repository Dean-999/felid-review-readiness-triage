#!/usr/bin/env python3
"""Evaluate held-out retrieval for Phase 11 pair-level PF-ERI runs."""
...
def evaluate(config: dict[str, Any], checkpoint: Path) -> None:
    print("WARNING: Phase 11 retrieval metrics are held-out pilot evidence only; no success claim follows.")
    ...
    train_rows = train_manifest[
        ...
        & (train_manifest["phase10_lite_plus_group"].astype(str) == str(config["base_group_name"]))
    ].copy()
    if train_rows.empty:
        raise ValueError("0 selected train rows found for split/base group")
    ...
    overlap = set(train_rows["identity_label_internal"].astype(str)) & set(test["identity_label_internal"].astype(str))
    if overlap:
        raise ValueError(f"train/test identity leakage: {len(overlap)} overlapping identities")
    ...
    for _, query in test.iterrows():
        ...
        for _, candidate in gallery.iterrows():
            ...
            sims.append((gid, float(np.dot(qvec, embeddings[gid])), same))
    ...
    pd.DataFrame([metrics]).to_csv(out_dir / "phase11_retrieval_metrics.csv", index=False)
    pd.DataFrame(case_rows).to_csv(out_dir / "phase11_failure_cases.csv", index=False)
