#!/usr/bin/env python3
"""Dry-run runner for Phase 11D-R revised positive rescue."""
...
def planned_record(root: Path, config_name: str, split_id: int) -> dict[str, object]:
    ...
    return {
        ...
        "positive_rescue_reliability_max": float(loss["positive_rescue_reliability_max"]),
        "positive_rescue_similarity_quantile": float(loss["positive_rescue_similarity_quantile"]),
        "positive_rescue_pattern_required": bool(loss["positive_rescue_pattern_required"]),
        "positive_rescue_pattern_min": float(loss["positive_rescue_pattern_min"]),
...
def main() -> None:
    ...
    print("Phase 11D-R revised positive-rescue planned runs:")
    ...
    for split_id in args.splits:
        for config_name in CONFIGS:
            ...
            print(
                ...
                f"R_max={record['positive_rescue_reliability_max']} "
                f"positive_q={record['positive_rescue_similarity_quantile']} "
                f"pattern_required={record['positive_rescue_pattern_required']} "
                f"floor={record['positive_rescue_floor']} "
    ...
    record_path = root / "phase11dr_revised_positive_rescue_run_records.json"
