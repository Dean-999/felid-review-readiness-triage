#!/usr/bin/env python3
"""Dry-run runner for Phase 11D-R2 target-rate positive rescue."""
...
def planned_record(root: Path, config_name: str, split_id: int) -> dict[str, object]:
    ...
    return {
        ...
        "target_rescue_fraction": float(loss["target_rescue_fraction"]),
        "target_rescue_reliability_max": float(loss["target_rescue_reliability_max"]),
        "target_rescue_floor": float(loss["target_rescue_floor"]),
        "target_rescue_score": str(loss["target_rescue_score"]),
...
def main() -> None:
    ...
    print("Phase 11D-R2 target-rate positive-rescue planned runs:")
    ...
    for split_id in args.splits:
        for config_name in CONFIGS:
            ...
            print(
                ...
                f"target_fraction={record['target_rescue_fraction']} "
                f"R_max={record['target_rescue_reliability_max']} "
                f"floor={record['target_rescue_floor']} "
    ...
    record_path = root / "phase11dr2_target_rate_positive_rescue_run_records.json"
