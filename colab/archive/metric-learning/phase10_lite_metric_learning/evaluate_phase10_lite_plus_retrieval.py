#!/usr/bin/env python3
"""Evaluate held-out retrieval for Phase 10-Lite Plus runs."""
...
def evaluate(config: dict[str, Any], checkpoint: Path) -> None:
    print("WARNING: Phase 10-Lite Plus retrieval metrics are held-out pilot evidence only.")
    ...
    train_rows = train_manifest[
        ...
        & (train_manifest["phase10_lite_plus_group"].astype(str) == str(config["phase10_lite_plus_group"]))
    ].copy()
    if train_rows.empty:
        raise ValueError("0 selected train rows found for split/group")
    ...
    pd.DataFrame([metrics]).to_csv(out_dir / "phase10_lite_plus_retrieval_metrics.csv", index=False)
    pd.DataFrame(case_rows).to_csv(out_dir / "phase10_lite_plus_failure_cases.csv", index=False)
